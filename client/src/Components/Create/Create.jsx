...
const Create = () => {
  ...
  const handleUserClick = async (userId) => {
    try {
      ...
      setSelectedApprentice((prevSelectedApprentice) => [...prevSelectedApprentice, ...res])
      setUserSearch([])
      setTeacherSearch([])
    } catch (error) {
  ...
  const deleteApprenticeSelected = (apprenticeId) => {
    setSelectedApprentice((prevSelectedApprentice) => prevSelectedApprentice.filter((apprentice) => apprentice.id_aprendiz !== apprenticeId))
  }
      ...
      <section className="w-full overflow-auto">
        ...
        <section className=" relative top-[1.6rem] place-items-center grid grid-cols-2  gap-0 ">
                  ...
                  {userSearch.length > 0 || selectedApprentice.length > 0 ? (
                    <>
                      ...
                      {selectedApprentice.map((item) => (
                        <ul className="flex justify-between text-[13px] py-[.5rem] cursor-pointer hover:bg-blue-900 rounded-lg p-2" key={item.id_aprendiz} onClick={() => deleteApprenticeSelected(item.id_aprendiz)}>
                          <React.Fragment>
                            <li>{item.numero_documento_aprendiz}</li>
                            <li>{item.nombres_aprendiz + ' ' + item.apellidos_aprendiz}</li>
                            <li>
          ...
          <section className="  absolute top-[27rem] ">
            <Button className="" size="lg" color="primary" onClick={sendData} isDisabled={selectedApprentice.length === 0}>
              Enviar
